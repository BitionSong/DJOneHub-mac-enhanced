# DJOneHub：让大疆第一代 4G 模块在 Mac 上管理实体 SIM

DJOneHub 基于 [ZenGeekLabs/DJOneHub](https://github.com/ZenGeekLabs/DJOneHub) 继续开发，把大疆第一代 4G 模块变成可在 Mac 上长期使用的实体 SIM 终端：4G 上网、短信、GPS、来电提醒和通话控制。

> [!IMPORTANT]
> **v1.0.0-rc1 目前只在本地准备和验证，尚未推送、打标签或创建 GitHub Release。**
> 当前候选产物包括 macOS Universal DMG 与 Windows amd64 ZIP；需经审核后才会公开。

> [!CAUTION]
> **公开源码版不包含模块侧语音运行时，也不会提供开箱可用的 Mac 双向通话音频。**
> 私有完整候选版中的双向语音依赖两份尚未取得对应源码或再分发授权的内核模块；在合规基础明确前，它们不会进入公开仓库或公开 Release。公开范围见 [OPEN_SOURCE_SCOPE.md](OPEN_SOURCE_SCOPE.md)。
...
| 功能 | 实现效果 |
| --- | --- |
| 通话控制 | 显示来电与通话状态，并提供拨号、接听、拒接、挂断、静音、DTMF、录音等 App 与接口实现。公开版不承诺双向语音可用。 |
| 独立 macOS App | SwiftUI 重做拨号、最近通话、短信、通讯录与设置，不再依赖浏览器完成日常操作。 |
| 来电与短信提醒 | 关闭管理页面后仍可显示原生风格提醒；短信支持验证码预览。 |
| 短信与网络同时在线 | 模块保持可管理状态时仍可提供 USB 4G；手动关闭 4G 不影响短信和来电监听。 |
| 网络策略 | macOS 优先使用 Wi-Fi，Wi-Fi 不可用时由 USB 4G 兜底，并自动修复网络服务与 DHCP。 |
| GPS | 本地读取 GNSS，提供菜单栏状态与地图浮窗；默认关闭。 |
| eSIM 与 AT | 管理兼容实体 eUICC Profile，并保留完整 AT 调试入口。 |

### 私有候选版的通话状态

私有候选版已经接入模块侧语音路由、USB Audio 8 kHz 单声道识别、Mac 输入输出路由和双向录音。部分通话仍可能出现轻微、间歇性的滋滋声；正常交流可用，但仍需继续优化采样时钟、缓冲和设备兼容性。

这不是公开版的功能承诺。公开源码会保留通话相关 App、状态管理和诊断代码，但不分发使模块暴露双向语音的内核运行时；缺少该运行时时，不能把 Mac 当作可正常通话的电话终端。

所有进一步测试都应保留短信、网络和现有通话的可回滚基线。未知私有命令不做猜测写入。

## 平台支持

| 平台 | 候选包 | 状态 |
| --- | --- | --- |
| Apple Silicon Mac | 私有 Universal DMG | 当前 Mac 已完成构建、自检与包内运行验证。公开 Release 尚未准备。 |
| Intel Mac | 私有 Universal DMG | 后端、App 与 libusb 均含 x86_64；尚未在真实 Intel Mac 上验证。公开 Release 尚未准备。 |
| Windows amd64 | 私有 ZIP + 安装/卸载脚本 | 可自动枚举 COM 口并打开独立 Edge App 窗口；尚未在真实 Windows + 模块上验证。公开 Release 尚未准备。 |

Windows 当前不等同于 macOS：双向通话音频、厂商 USB AT/eSIM、自动 USB 网卡策略、系统通讯录和 MapKit 仍是 macOS 专用。Windows 控制台会隐藏这些不可用功能，不会误报已支持。

程序只监听 `127.0.0.1:7575`，不会主动把 SIM、短信、联系人、录音或卡片资料上传到远程服务器。
...
| Mac 双向通话 | 私有候选已实现 | 依赖未公开分发的模块侧语音运行时；公开版不承诺可用，仍有轻微偶发底噪待优化 |
...
- 实体 SIM 来电和新短信会通过常驻的 macOS App 显示；来电可接听、拒接或挂断。
- 4G 拨块可强制禁止 Mac 使用蜂窝数据，同时保留短信与来电监听。
- 4G 被关闭时，网页流量读数会清空，避免把历史数值误认为仍在使用蜂窝网络。

- 私有候选版在通话期间会启动模块侧语音路由与 Mac CoreAudio 路由，并在结束后恢复设备状态。该模块侧运行时不属于公开分发范围。
...
| 功能 | 对应源码 |
| --- | --- |
| 来电状态、拨号、接听、DTMF、录音与历史记录 | [`calls.go`](cmd/djonehub-macos/calls.go)、[`audio_darwin.go`](cmd/djonehub-macos/audio_darwin.go)、[`module_voice_public.go`](cmd/djonehub-macos/module_voice_public.go) |
...
| 独立 macOS App、来电/短信提醒、通讯录与设置 | [`macOS SwiftUI App`](macos/DJOneHubNotifier) |
...
- 支持数据传输的 USB-C 线缆
- Apple Silicon 或 Intel Mac；Windows 版需 Windows amd64 PC 与可用串口驱动
...
- macOS Universal 候选包同时包含 arm64 与 x86_64
- Intel Mac 与 Windows 版本仍需对应真机验证
...
v1.0.0-rc1 仍处于私有审核阶段，以下文件只在本地生成，尚未上传：

- `DJOneHub-macOS-universal-v1.0.0-rc1.dmg`：Apple Silicon 与 Intel Mac 通用，包含后端、独立 App、图标、安装与卸载脚本。
- `DJOneHub-Windows-amd64-v1.0.0-rc1.zip`：Windows amd64，包含 GUI 入口 EXE、控制台、安装/卸载与停止脚本。

审核通过后再创建正式 Release；历史预览版仍会保留，不覆盖旧文件。

Release 页面还会提供同名的 `.sha256` 文件。它不是程序的一部分，也不是安装必需文件，仅用于确认 DMG 或 ZIP 是否下载完整、是否与发布者生成的文件一致。
...
macOS：打开 DMG，双击“安装 DJOneHub.command”，按提示完成安装。安装包同时包含 Apple Silicon 与 Intel 架构。

Windows：完整解压 ZIP，双击 `Install DJOneHub.cmd`。安装后从开始菜单启动 DJOneHub。
...
- Apple Silicon Mac（用于生成 macOS Universal 包）
- macOS 13 或更新版本
...
- Universal 包已包含 Apple Silicon 与 Intel 架构，但 Intel 尚未真机验证。
- Windows amd64 包已完成交叉编译和静态检查，但尚未在真实 Windows + 模块环境验证；通话音频、厂商 USB AT/eSIM、自动 USB 网卡策略、系统通讯录与 MapKit 不可用。
- 模式 2 和模式 3 尚未确定稳定用途。
...
本项目旨在研究、学习和个人设备扩展用途，用于实现兼容硬件模块在计算机系统中的通信能力。

本项目仅提供硬件接口适配、驱动支持及基础通信功能，包括但不限于网络连接、电话、短信等设备原生能力。

使用者需确保自己的使用行为符合当地法律法规及运营商服务协议。

开发者不参与、不支持、不鼓励任何形式的骚扰通信、垃圾信息发送、诈骗、未经授权的数据收集或其他违法活动。

因使用者对本项目的二次修改、扩展或实际使用行为产生的任何法律责任，由使用者自行承担。
...
计划公开的源码版本将继续遵循 [PolyForm Noncommercial License 1.0.0](LICENSE)，仅允许许可证定义的非商业用途；不会把未满足对应源码条件的模块二进制纳入公开仓库或公开安装包。
